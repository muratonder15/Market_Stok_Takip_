...
namespace Market_Stok_Takip_MRT
{
    public partial class Raporlar : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            textBox15.Text = baglanti.verileriOku("select format(sum(toplam_tutar),'Currency') from hareketler where islem_turu_kodu=1 and hareket_turu_kodu=5 and odeme_turu_kodu=2 and islem_tarihi>=CDATE('" + dateTimePicker1.Value.ToShortDateString() + " 00:00:00') and islem_tarihi<=CDATE('" + dateTimePicker2.Value.ToShortDateString() + " 23:59:59')").Rows[0][0].ToString();// toptancıya iade
            textBox16.Text = baglanti.verileriOku("select format(sum(toplam_tutar),'Currency') from hareketler where islem_turu_kodu=2 and hareket_turu_kodu=5 and odeme_turu_kodu=2 and islem_tarihi>=CDATE('" + dateTimePicker1.Value.ToShortDateString() + " 00:00:00') and islem_tarihi<=CDATE('" + dateTimePicker2.Value.ToShortDateString() + " 23:59:59')").Rows[0][0].ToString();
